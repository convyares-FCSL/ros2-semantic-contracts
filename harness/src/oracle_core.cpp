...
int run_oracle(const RunConfig& cfg, BackendKind backend) {
  ...
  // --- Run metadata (authoritative) ---
  {
    auto ev = base_event(cfg.version, cfg.run_id, "run_start", "_run");

    json detail = json::object({
        {"backend", backend == BackendKind::Ros ? "ros" : "stub"},
    });

    // Only include ROS environment when backend is ROS; this avoids confusing “stub”
    // runs on machines that happen to have ROS env set.
    if (backend == BackendKind::Ros) {
      detail["ros_distro"] = getenv_or("ROS_DISTRO", "unknown");
      // RMW_IMPLEMENTATION may be unset; treat that as a meaningful, auditable value.
      detail["rmw_implementation"] = getenv_or("RMW_IMPLEMENTATION", "default");
    }
  ...
  for (const auto& sc : root["scenarios"]) {
    ...
    // spec_id optional; fallback to scenario id (used only for trace context)
    ...
    // --- Execution state (STUB MODEL, pre-ROS) ---
